...
class SegmentationService:
    ...
    async def append_lora_entry(self, session_id: str, segment_index: int, original_prompt: str) -> Dict[str, Any]:
        ...
        system_prompt = (
            "You are an expert vision-language data annotation engine designed to create high-quality captions "
            "for training LoRA (Low-Rank Adaptation) models.\n\n"
            "INPUTS PROVIDED:\n"
            "1. IMAGE_A: The original, unaltered full image.\n"
            "2. IMAGE_B: The isolated object/segment extracted from the original image (via SAM).\n"
            f"3. IDENTIFYING_PROMPT: \"{original_prompt}\"\n\n"
            "YOUR TASK:\n"
            "1. Examine IMAGE_A to understand the broader context, lighting, style, and setting.\n"
            "2. Examine IMAGE_B to inspect the fine details, texture, color, and unique characteristics of the specific target object.\n"
            "3. Cross-reference these with the IDENTIFYING_PROMPT to ensure absolute alignment.\n"
            "4. Generate a highly descriptive caption suitable for LoRA training that captures the specific essence "
            "of the isolated object while maintaining context from the scene.\n\n"
            "OUTPUT FORMAT CONSTRAINTS:\n"
            "- You must output exactly ONE line.\n"
            "- The line must be a single, valid JSON object.\n"
            "- Do NOT wrap the JSON in markdown code blocks.\n"
            "- Do NOT include any introductory text, pleasantries, explanations, or trailing characters.\n"
            "- If your output violates this format, the data pipeline will break.\n\n"
            "REQUIRED JSON SCHEMA:\n"
            "{\n"
            f"  \"file_name\": \"{file_name}\",\n"
            "  \"text\": \"The highly descriptive training caption generated by analyzing the inputs\"\n"
            "}"
        )
        response = await gemini_model.generate_content_async([system_prompt, original_img, segment_img])

        raw = response.text.strip()
        # Strip markdown fences if Gemini ignores the constraint
        raw = re.sub(r'^```[a-zA-Z]*\n?', '', raw)
        raw = re.sub(r'\n?```$', '', raw.strip())
        parsed = json.loads(raw.strip())
        refined_prompt = parsed.get("text", "").strip()
